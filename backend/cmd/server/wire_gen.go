...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	// QuotaShareService: manual wiring (not Wire-managed) for quota_share subscription distribution
	quotaShareCache := repository.NewQuotaShareCache(redisClient)
	quotaShareGroupRepo := repository.NewQuotaShareGroupRepository(client, db)
	quotaShareKeyRepo := repository.NewQuotaShareKeyRepository(client, db)
	quotaShareService := service.NewQuotaShareService(quotaShareCache, quotaShareGroupRepo, quotaShareKeyRepo, usageLogRepository)
	billingCacheService.SetQuotaShareService(quotaShareService)
	gatewayService.SetQuotaShareService(quotaShareService)
	openAIGatewayService.SetQuotaShareService(quotaShareService)
	gatewayHandler.SetQuotaShareService(quotaShareService)
	adminService.SetQuotaShareService(quotaShareService)
	apiKeyService.SetQuotaShareService(quotaShareService)
